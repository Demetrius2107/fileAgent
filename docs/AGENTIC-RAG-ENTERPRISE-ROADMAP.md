...
- 查询分类：无需检索、单跳检索、并列多问题 `MULTI_QUERY`、依赖式多跳 `MULTI_HOP`、比较、聚合、时间敏感。
...
- 红队样本覆盖 OWASP Agentic 风险和常见 RAG Injection（先攻击再防御）。
...
**目标**：任何一次错误回答、质量退化、失败动作或费用异常都能被定位、解释并进入持续改进闭环。
...
Phase 0 的固定数据集和离线门禁用于上线前回归；本阶段补充面向真实流量的持续评估，用来发现问题分布变化、文档更新、模型切换和索引变更造成的线上质量退化。

- 问答完成后按可配置比例抽样，通过独立异步任务执行 Judge，不阻塞 SSE，不改变用户请求的成功或失败结果。
- 增加点赞、点踩和用户纠正反馈；点踩和纠正属于强信号，不参与随机抽样，必须进入待复核队列。
- 在线样本至少关联问题、最终答案、检索证据、最终引用、租户、模型版本、Prompt 版本、索引版本、`traceId` 和发生时间；内容按 Phase 7 的权限、脱敏与保留策略存储。
- 优先评估不依赖人工标准答案的指标：忠实度、答案相关性、引用支持度和拒答合理性；需要标准答案的 Recall@K、MRR、事实完整率等继续由 Phase 0 离线评测承担。
- Judge 分数只作为趋势、告警和人工复核信号，不作为单条答案正确性的事实判定，也不得直接阻断在线回答。
- 低分、错误引用、异常拒答和反馈样本进入持久化坏答案队列，支持去重、优先级、处理状态、责任人和复核结论，不使用本地 JSONL 作为生产存储。
- 按租户、数据集、模型、Prompt 和索引版本统计分数分布与变化趋势；样本量达到最低门槛后再触发质量告警，避免小样本波动造成误报。
- 人工确认的坏案例分类为检索、生成、引用、数据、权限或产品问题；有代表性的案例回灌 Phase 0 固定评测集，并通过离线回归验证修复效果。
- 在线评估 Worker 必须与问答链路资源隔离，具备限流、预算、重试、死信和暂停开关；Judge 不可用时只记录评估失败，不影响主业务。
...
- 质量看板：在线评估趋势、反馈、无答案、低置信度、错误引用和用户纠正；支持坏答案队列复核、归因、责任人流转和样本回灌。
